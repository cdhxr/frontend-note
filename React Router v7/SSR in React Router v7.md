...
1. **它是一个 HTTP 客户端**：  
    fetch 是浏览器内置的一个功能，它的唯一工作就是向一个给定的 **URL** 发送一个 **HTTP 请求**，然后等待并处理服务器返回的 **HTTP 响应**。
    
2. **它需要一个“地址”**：  
    就像你寄信需要一个收信地址一样，fetch 必须知道要把请求发送到哪里去。这个“地址”就是一个 URL，例如 https://api.example.com/users 或 /api/products/123。
    
3. **它“看不见”服务器内部**：  
    客户端的 JavaScript 代码运行在用户的浏览器里，它与你的服务器是完全隔离的。它不知道你的服务器上有什么文件、哪个函数、更不可能知道如何连接数据库。它唯一能与服务器沟通的方式，就是通过公开的、可访问的 HTTP URL。

### API 层的角色：提供这个“地址”

API 层（无论是 REST API、GraphQL API，还是 Server Actions 自动生成的端点）的核心职责之一，就是**创建并监听这些 URL 地址**。

SSR简化了API层的构建

事实上是现代框架隐性的构造了API层
它们是你应用的前端与数据源之间的接口。
你不需要再单独创建 /api/posts/:postId 这样的 REST 端点。

它是一种**更高层次的抽象**，将 API 的创建、调用和后续的 UI 更新流程自动化、集成化了。
